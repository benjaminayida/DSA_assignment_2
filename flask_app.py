...
from helper import perform_calculation, convert_to_float, Circle
    ...
    if request.method == 'POST':
        ...
        if operation not in ['add', 'subtract', 'divide', 'multiply']:
            return render_template('calculator.html',
                                   printed_result='Operation must be one of "add", '
    ...
    if request.method == 'POST':
        ...
        if operation not in ['perimeter', 'area']:
            return render_template('circle.html',
                                   printed_result='Operation must be one of "Area", or "Perimeter".')
        try:
        ...
        try:
            result = circle.circle_calculate(operation)
            return render_template('circle.html', printed_result=str(result))

        except ValueError:
            return render_template('circle.html', error="Invalid input. Please enter a valid number.")
